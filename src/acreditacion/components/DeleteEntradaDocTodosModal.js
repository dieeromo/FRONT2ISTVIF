...
import {useDeleteEntradaDocumentoMutation} from '../services/criteriosApi'
const DeleteEntradaDocTodosModal = ({id}) => {
   
    const user = JSON.parse(localStorage.getItem('user') || "{}")  // para [asar las credenciales]
    const [deleteEntradaDocumento] = useDeleteEntradaDocumentoMutation()
    ...
    const deleteModal = (e) => {
        deleteEntradaDocumento([user.access, id])
        closeModal()
    };
...
export default DeleteEntradaDocTodosModal ;
